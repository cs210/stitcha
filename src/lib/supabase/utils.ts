...
export async function handleProductTableInsert(formData: FormData, supabase: SupabaseClient, userId: string) {
	...
	const productData: ProductFormData = {
		...
		inmetro_cert_number: JSON.parse(formData.get('inmetro_cert_number') as string),
		barcode: JSON.parse(formData.get('barcode') as string),
		...
		product_type: JSON.parse(formData.get('product_type') as string),
	...
	// Check if a product with the same system_code exists
	...
	if (imageFiles && imageFiles.length > 0) {
		...
		for (const file of imageFiles) { // promise all method 
			...
			const { data: urlData, error: urlError } = await supabase.storage.from('products').createSignedUrl(filePath, 60 * 60 * 24 * 365); // 1 year expiration
	...
	if (technical_sheet_url) {
		...
		const { error: uploadError } = await supabase.storage.from('technical-sheets').upload(filePath, technical_sheet_url, {
			contentType: technical_sheet_url.type,
		});
		...
		const { data: urlData, error: urlError } = await supabase.storage.from('technical-sheets').createSignedUrl(filePath, 60 * 60 * 24 * 7); // 7 days expiration

		// Check if the URL was successfully generated
		if (urlError) {
			throw new Error(`Failed to generate public URL for technical sheet: ${urlError.message}`);
		}
...
export async function updateLaborFromProduct(productId: string, supabase: SupabaseClient, formData: FormData) {
	...
	for (const labor of labors) {
		...
		const { data: insertedTask, error: insertError } = await supabase	// upsert method
			.from('labor_types')
			.upsert(task_insert, {
				onConflict: 'task',
