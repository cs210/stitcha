...
async function captureElement(element: HTMLElement): Promise<{ dataUrl: string; canvas: HTMLCanvasElement }> {
	...
	const fullHeight = Math.max(element.scrollHeight, element.offsetHeight, element.clientHeight, rect.height, maxBottom);
	const fullWidth = Math.max(element.scrollWidth, element.offsetWidth, element.clientWidth, rect.width, maxRight);
...
export async function generateProductPDF(productImageRef: HTMLElement, productDetailsRef: HTMLElement, product: Product): Promise<Uint8Array> {
	try {
		...
		const detailsLeftMargin = finalDetailsWidth < maxWidth ? margin + (maxWidth - finalDetailsWidth) / 2 : margin;
