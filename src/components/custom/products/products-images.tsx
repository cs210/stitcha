					...
					product.image_urls.map((imageUrl: string, index: number) => (
						<CarouselItem key={index}>
							<div className="w-full h-full flex items-center justify-center">
								<Image src={imageUrl} alt={`${dict.product.images.productImage} ${index + 1}`} className="w-96 h-96 object-contain" width={96} height={96} />
							</div>
						</CarouselItem>
					))
				) : (
					<CarouselItem>
						<P>{dict.product.images.noImagesAvailable}</P>
					</CarouselItem>
				)}
			</CarouselContent>
			<CarouselPrevious />
