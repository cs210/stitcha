...
export default function Page({ params }: EditPageProps) {
	...
	const [formData, setFormData] = useState(() => {
		...
		for (const column of Object.values(savedProducts) as any[][]) {
			const found = column.find((p: any) => p.id === id);
			if (found) {
				product = found;
				break;
			}
			...
			<div className='grid grid-cols-2 gap-6'>
						...
						<div className='flex items-center justify-between text-sm text-gray-500 mt-2'>
							...
							{/* {selectedSeamstress && (
								<div className='flex -space-x-2'>
									<Image src={selectedSeamstress.avatar} alt={selectedSeamstress.name} width={24} height={24} className='rounded-full border-2 border-white' />
								</div>
							)} */}
						</div>
				...
				<div className='space-y-6'>
					...
					<div>
						...
						<Select value={formData.seamstress} onValueChange={(value) => setFormData((prev) => ({ ...prev, seamstress: value }))}>
							<SelectTrigger>
								...
								{/* {seamstresses.map((seamstress) => (
									<SelectItem key={seamstress.id} value={seamstress.id.toString()}>
										<div className='flex items-center gap-2'>
											<Image src={seamstress.avatar} alt={seamstress.name} width={24} height={24} className='rounded-full' />
											{seamstress.name}
										</div>
									</SelectItem>
								))} */}
					...
					<div>
						...
						<Input
							...
							onChange={(e: ChangeEvent<HTMLInputElement>) => setFormData((prev) => ({ ...prev, quantity: e.target.value }))}
						/>
					</div>
					...
					<div>
						...
						<div className='mt-2'>
							<input type='file' ref={techSheetsInputRef} onChange={(e) => handleFileUpload(e, false)} accept='image/*,.pdf' multiple className='hidden' />
							<Button variant='outline' onClick={() => techSheetsInputRef.current?.click()} className='flex items-center gap-2'>
					...
					<div>
						...
						<div className='mt-2'>
							<input type='file' ref={imageInputRef} onChange={(e) => handleFileUpload(e, true)} accept='image/*' className='hidden' />
							<Button variant='outline' onClick={() => imageInputRef.current?.click()} className='flex items-center gap-2'>
								<Upload className='w-4 h-4' />
								Choose File
							</Button>
							<span className='ml-2 text-gray-500'>{imagePreview !== '/images/tote.png' ? 'Image selected' : 'No File Chosen'}</span>
